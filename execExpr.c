		...
		case T_Aggref:
			{
				...
				scratch.opcode = EEOP_OMRJITCOMPILE_EXPR;
				ExprEvalPushStep(state, &scratch);
			 ...
			 Oid inputcollid, ExprState *state)
{
	...
	/* Set up the primary fmgr lookup information */
	//elog(INFO,"check fbp->funcName---------------------------------------------------------------- \n");
	scratch->operatorId = funcid;
	...
	foreach(lc, args)
	{
		...
		if (IsA(arg, Const))
		{
			...
			//elog(INFO,"con->constvalue ---------------------------------------------------------------- %d \n", fcinfo->args[argno].value);
			//elog(INFO,"con->constvalue ---------------------------------------------------------------- %d \n", fcinfo->args[argno].isnull);
		}
		else
		{
			//elog(INFO,"&fcinfo->args[argno].value ---------------------------------------------------------------- %d \n", fcinfo->args[argno].value);
			//elog(INFO,"&fcinfo->args[argno].isnull ---------------------------------------------------------------- %d \n", fcinfo->args[argno].isnull);
