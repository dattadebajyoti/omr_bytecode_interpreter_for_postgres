...
static bool call_omr_jit(ExprState *state);
static Datum ExecRunOMRCompiledExpr(ExprState *state, ExprContext *econtext, bool *isNull);
static bool omr_bytecode_interpreter(ExprState *state);
...
{
	if (/*jit_compile_expr(state)*/ call_omr_jit(state))
		return;

	ExecReadyInterpretedExpr(state);
}

bool call_omr_jit(ExprState *state)
{
	if (!state->parent)
		return false;
	...
	if (omr_bytecode_interpreter(state))
	{
		return true;
	}
	return false;
...
static bool omr_bytecode_interpreter(ExprState *state)
{
	/*state->evalfunc = ExecRunOMRCompiledExpr;
...
static Datum
ExecRunOMRCompiledExpr(ExprState *state, ExprContext *econtext, bool *isNull)
{
	...
	state->evalfunc = (ExprStateEvalFunc)omr_vm;

	return (ExprStateEvalFunc)(*omr_vm)(state, econtext, isNull);
}
